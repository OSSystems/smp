...
class ForwardTree:
    ...
    def _get_int(nimbles: List[int]) -> int:
        ...
        hops = nimbles[0] if nimbles else 0
        if hops > 15:
    ...
    @staticmethod
    def _get_ft(value: int) -> List[int]:
        """Reconstruct [hops, port_1, ..., port_hops] from the 64-bit value.

        This is the exact inverse of `_get_int`: hops live in nibble 15, and the
        per-hop port for hop `h` lives in nibble `h - 1`.
        """
        hops = ForwardTree._get_nibble(15, value)
        return [hops] + [ForwardTree._get_nibble(hops - i - 1, value) for i in range(hops)]
    ...
    def loads(forward: bytes) -> 'ForwardTree':
        ...
        return ForwardTree(ForwardTree._get_ft(data64))
